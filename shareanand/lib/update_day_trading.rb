...
#down_url="http://www.nseindia.com/content/historical/EQUITIES/#{csv_yr}/#{mnth}/#{file_name}"
down_url="http://www.nseindia.com/content/historical/EQUITIES/2015/AUG/cm04AUG2015bhav.csv.zip"
#puts down_url
...
def self.insert_lines_following_line(file,line_no)
  tmp_fn = "#{file}.tmp"
...
def self.risk_return(data,x)
  stats = DescriptiveStatistics::Stats.new(data)
  if stats.standard_deviation.nil? 
    mean = nil
    variance = nil
    sigma = nil
    z = nil
    ...
    if z < 0 && z > -3
      probility = ZTable.find_by_z_value(z.abs).area.to_f
    elsif z > 0 && z < 3
      probility = 1 - ZTable.find_by_z_value(z).area.to_f
    elsif z > 3
      probility = 0
    elsif z < -3
      probility = 2
...
csv_file='/home/sradhu/risk_prgm/cm01OCT2015bhav.csv'
fil.puts csv_file
...
updated_low = arra[4].to_f < daytrading.f2_week_low ? arra[4].to_f : daytrading.f2_week_low
updated_high = arra[3].to_f > daytrading.f2_week_high ? arra[3].to_f : daytrading.f2_week_high
...
DayTrading.find(symb).update( 
    ...
    :f2_week_low => updated_low,
    :f2_week_high => updated_high,
